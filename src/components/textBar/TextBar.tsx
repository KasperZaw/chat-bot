import "./text-bar.scss";
import microfonIcon from "../../assets/icons/icon-microfone.svg";
import { useState } from "react";

interface TextBarProps {
  onValueSubmit: (value: string) => void;
  isLoading: boolean;
}
const TextBar = ({ onValueSubmit, isLoading }: TextBarProps) => {
  const [message, setMessage] = useState("");

  const sendToParent = () => {
    onValueSubmit(message);
    setMessage("");
  };
  return (
    <div className="text-bar-container">
      <div>
        <button>
          <img src={microfonIcon} alt="microfone" />
        </button>
        <input
          type="text"
          placeholder="Ask anything from here"
          value={message}
          onChange={(e) => setMessage(e.target.value)}
          onKeyDown={(e) => {
            if (e.key === "Enter" && !(message.trim() === "" || isLoading)) {
              sendToParent();
            }
          }}
        />
        <button
          className="send-btn"
          disabled={message.trim() === "" || isLoading}
          onClick={sendToParent}
        >
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10.9136 13.0853C10.7224 12.8945 10.4947 12.7444 10.244 12.6441L2.31399 9.46406C2.21931 9.42606 2.13851 9.36002 2.08245 9.27478C2.02638 9.18955 1.99773 9.0892 2.00035 8.98722C2.00296 8.88523 2.03671 8.78648 2.09706 8.70423C2.15741 8.62197 2.24148 8.56015 2.33799 8.52706L21.338 2.02706C21.4266 1.99505 21.5225 1.98894 21.6144 2.00945C21.7064 2.02995 21.7906 2.07622 21.8572 2.14283C21.9238 2.20945 21.9701 2.29366 21.9906 2.38561C22.0111 2.47756 22.005 2.57345 21.973 2.66206L15.473 21.6621C15.4399 21.7586 15.3781 21.8426 15.2958 21.903C15.2136 21.9633 15.1148 21.9971 15.0128 21.9997C14.9108 22.0023 14.8105 21.9737 14.7253 21.9176C14.64 21.8615 14.574 21.7807 14.536 21.6861L11.356 13.7541C11.2552 13.5036 11.1047 13.2761 10.9136 13.0853ZM10.9136 13.0853L21.854 2.14706"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default TextBar;
